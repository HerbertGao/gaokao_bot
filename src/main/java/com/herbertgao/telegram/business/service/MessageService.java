        ...
        if (TelegramBotUtil.isUserChat(message)) {
            ...
            if (userTemplateList.size() == 0) {
                sb.append("您尚未添加自定义模板。\n")
                        .append("添加自定义模板后，您可在发送框中输入@").append(Config.getUsername()).append("发送该消息。\n")
                        .append("\n您可使用 ").append(Command.ADD_COMMAND).append(" 命令添加at的用户，格式为：\n")
                        .append(Command.ADD_COMMAND).append("@").append(Config.getUsername()).append(" username \n")
                        .append("\n例如：\n")
                        .append(Command.ADD_COMMAND).append("@").append(Config.getUsername()).append(" HerbertGao \n");
            } else {
            ...
            return "请私聊Bot执行此命令。 @" + Config.getUsername();
        }
    }
        ...
        if (TelegramBotUtil.isUserChat(message)) {
            ...
            if (StringUtils.isNotBlank(text)) {
                ...
                sb.append("已添加 @").append(text).append(" 到at名单中。\n")
                        .append("您可在发送框中输入@").append(Config.getUsername()).append("发送该消息。\n");
            } else {
        ...
        if (TelegramBotUtil.isUserChat(message)) {
            ...
            if (text.length() > TEMPLATE_MAX_LENGTH) {
                ...
                sb.append("请在命令后跟随要添加的自定义模板，格式为：\n")
                        .append(Command.CUSTOMIZE_COMMAND).append("@").append(Config.getUsername()).append(" 模板 \n");
                sb.append("\n例如：\n")
                        .append(Command.CUSTOMIZE_COMMAND).append("@").append(Config.getUsername()).append(" 现在距离{exam}还有{time}\n" +
                        "@HerbertGao \n");
                sb.append("\n 参数：\n {exam} : 考试名称 \n {time} : 倒计时时间");
